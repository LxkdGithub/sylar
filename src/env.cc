/*
 * @Author: lxk
 * @Date: 2022-10-11 15:06:15
 * @LastEditors: lxk
 * @LastEditTime: 2022-10-11 15:31:12
 */
#include "env.h"
#include "config.h"
#include "log.h"
...
bool Env::init(int argc, char **argv) {
  ...
  auto rt = readlink(link, path, sizeof(path));
  if (rt == -1) {
    return false;
  }
  // /path/xxx/exe
...
std::string Env::getAbsoluteWorkPath(const std::string &path) const {
  ...
  static spadger::ConfigVar<std::string>::ptr g_server_work_path =
      spadger::Config::Lookup<std::string>("server.work_path");
