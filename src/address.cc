/*
 * @Author: lxk
 * @Date: 2022-10-20 20:34:00
 * @LastEditors: lxk
 * @LastEditTime: 2022-10-22 19:30:26
 */
#include "address.h"
#include "log.h"
...
/**
 * @brief Create a mask with the last bits is 1.
 *
 */
template <typename T> static T CreateMask(uint32_t bits) {
  return (1 << (sizeof(T) * 4 - bits)) - 1;
}

template <typename T> static uint32_t CountBytes(T value) {
  uint32_t result = 0;
  for (; value; ++result) {
    value &= value - 1;
  }
...
bool Address::GetInterfaceAddresses(
  ...
  if (getifaddrs(&results) != 0) {
    SPADGER_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
                                   " err="
  ...
  try {
    ...
    SPADGER_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
    freeifaddrs(results);
    return false;
  }
  freeifaddrs(results);
...
bool Address::operator==(const Address &rhs) const {
  ...
  if (memcmp(getAddr(), rhs.getAddr(), getAddrLen()) != 0) {
    return false;
  }
  return true;
}
...
IPAddress::ptr IPAddress::Create(const char *address, uint16_t port) {
  ...
  if (error) {
    SPADGER_LOG_ERROR(g_logger)
        << "IPAddress::Create(" << address << ", " << port << ") error" << error
...
IPv6Address::ptr IPv6Address::Create(const char *address, uint16_t port) {
  ...
  if (result <= 0) { // return 1 on success, return 0/-1 on error
    SPADGER_LOG_ERROR(g_logger)
...
IPAddress::ptr IPv6Address::broadcastAddress(uint32_t prefix_len) {
  ...
  // 因为小端(地位在前，高位在后)的原因，所以向后写
  for (int i = prefix_len / +1; i < 16; i++) {
...
/*
    struct sockaddr_un {
        sa_family sun_family; // PF_UNIX | AF_UNIX
        char sun_apth[UNIX_PATH_MAX/108];
    }
*/
/*
    注意下面((sockaddr_un*)0)->sun_path的用法
    可以使用这种方式计算字段在结构体内的偏移:
    void *offset = &(((sockaddr_un *)0)->sun_path);
*/
...
std::ostream &UnixAddress::insert(std::ostream &os) const {
  ...
  return os << m_addr.sun_path; // 为什么输出两次啊
}
